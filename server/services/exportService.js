...
class ExportService {
  ...
  static async generateSalesPDF(filters = {}, options = {}) {
    try {
      // options.userId scopes the report to the requesting seller (data isolation)
      const analytics = await AnalyticsService.getSalesAnalytics(filters, options.userId);
      ...
      const doc = new PDFDocument({
        ...
        info: {
          Title: 'Sales Report',
          Author: 'E-Shop by BHABY GROUP LTD',
          Subject: 'Sales Analytics Report',
          Creator: 'E-Shop Retail Management System'
        }
      });
  ...
  static async generateSalesExcel(filters = {}, options = {}) {
    try {
      ...
      workbook.creator = 'E-Shop by BHABY GROUP LTD';
  ...
  static async generateInventoryPDF(filters = {}) {
    try {
      // filters.userId scopes the report to the requesting seller (data isolation)
      const analytics = await AnalyticsService.getInventoryAnalytics(filters, filters.userId);
  ...
  static addPDFHeader(doc, title, filters = {}) {
    ...
    doc.fontSize(20)
       .fillColor('#2563eb')
       .text('E-SHOP BY BHABY GROUP LTD', 50, 50);
  ...
  static addSalesSummaryToPDF(doc, summary) {
    ...
    const summaryData = [
      ['Total Revenue', `TZS ${summary.totalRevenue?.toLocaleString() || '0'}`],
  ...
  static addSalesChartsToPDF(doc, analytics) {
    ...
    if (analytics.dailyTrend && analytics.dailyTrend.length > 0) {
      ...
      analytics.dailyTrend.slice(-7).forEach(day => {
        doc.text(`${day.date}: TZS ${day.revenue?.toLocaleString() || '0'} (${day.sales || 0} sales)`, 70, yPosition);
        yPosition += 15;
      });
    }
  }
    ...
    if (analytics.topProducts && analytics.topProducts.length > 0) {
      ...
      analytics.topProducts.slice(0, 10).forEach(product => {
        doc.text(product.productName || 'Unknown', 50, yPosition)
           ...
           .text(`TZS ${product.totalRevenue?.toLocaleString() || '0'}`, 300, yPosition);
        yPosition += 15;
      });
    }
  }
  ...
  static addPDFFooter(doc) {
    // bufferedPageRange() returns 1-based { start, count } — switchToPage takes
    // the 1-based page number, so 0 is always out of bounds.
    ...
    for (let i = start; i < start + count; i++) {
      ...
      doc.fontSize(8)
         .fillColor('#9ca3af')
         .text(`Page ${i - start + 1} of ${count}`, 50, 750)
         .text('Generated by E-Shop by BHABY GROUP LTD', 350, 750);
    }
  }
  ...
  static addSalesSummaryToExcel(worksheet, summary, filters) {
    ...
    summaryData.forEach(([metric, value]) => {
      ...
      if (row === 6) {
        ...
        // Currency formatting
        worksheet.getCell(`B${row}`).numFmt = '"TZS "#,##0.00';
      } else if (typeof value === 'number' && metric.includes('%')) {
  ...
  static addTopProductsToExcel(worksheet, topProducts) {
    ...
    topProducts.forEach((product, index) => {
      ...
      // Format currency
      worksheet.getCell(`C${row}`).numFmt = '"TZS "#,##0.00';
    });
  ...
  static addStaffPerformanceToExcel(worksheet, staffPerformance) {
    ...
    staffPerformance.forEach((staff, index) => {
      ...
      // Format currency
      worksheet.getCell(`C${row}`).numFmt = '"TZS "#,##0.00';
      worksheet.getCell(`D${row}`).numFmt = '"TZS "#,##0.00';
    });
  ...
  static addInventorySummaryToPDF(doc, stockOverview) {
    ...
    const inventoryData = [
      ...
      ['Total Stock Value', `TZS ${stockOverview.totalStockValue?.toLocaleString() || '0'}`],
