 ...
 *
 * Desktop: deep blue→purple branded left panel with floating marketplace
 * icons + selling points, and the form card on the right.
 * Mobile: compact branded header with the same floating icons above the card.
 *
 * The icon field is decorative (aria-hidden) and cheap to run: ~10 absolutely
 * positioned icons using one shared `float` keyframe with staggered delays.
 * All motion is disabled under prefers-reduced-motion (Tailwind's motion-safe).
 */
...
/** Floating icon spec: position, size, delay, duration, tint. */
const FLOATING_ICONS = [
  { Icon: ShoppingBag,  left: '8%',  top: '12%', size: 34, delay: '0s',     duration: '9s',  color: 'text-sky-300/50', ring: true },
  { Icon: Store,        left: '72%', top: '18%', size: 40, delay: '1.2s',   duration: '11s', color: 'text-indigo-300/40', ring: false },
  { Icon: Package,      left: '16%', top: '48%', size: 30, delay: '2.1s',   duration: '8s',  color: 'text-white/35', ring: true },
  { Icon: Truck,        left: '82%', top: '58%', size: 36, delay: '0.6s',   duration: '12s', color: 'text-sky-300/40', ring: false },
  { Icon: CreditCard,   left: '6%',  top: '74%', size: 28, delay: '1.8s',   duration: '9.5s', color: 'text-white/35', ring: false },
  { Icon: Smartphone,   left: '64%', top: '80%', size: 32, delay: '2.8s',   duration: '10s', color: 'text-indigo-300/50', ring: true },
  { Icon: BarChart3,    left: '48%', top: '8%',  size: 30, delay: '3.4s',   duration: '8.5s', color: 'text-white/30', ring: false },
  { Icon: ShieldCheck,  left: '88%', top: '36%', size: 28, delay: '1.5s',   duration: '11.5s', color: 'text-emerald-300/40', ring: false },
  { Icon: Sparkles,     left: '30%', top: '26%', size: 26, delay: '2.6s',   duration: '7.5s', color: 'text-amber-300/40', ring: false },
  { Icon: Users,        left: '42%', top: '66%', size: 30, delay: '0.9s',   duration: '10.5s', color: 'text-sky-300/35', ring: true },
];
...
function FloatingIconField({ className = '' }: { className?: string }) {
  return (
    <div aria-hidden="true" className={`pointer-events-none absolute inset-0 overflow-hidden ${className}`}>
      ...
      {FLOATING_ICONS.map(({ Icon, left, top, size, delay, duration, color, ring }, i) => (
        <span
          key={i}
  ...
  return (
    <div className="min-h-screen lg:grid lg:grid-cols-[1.1fr_1fr] bg-slate-50">
      ...
      <div className="relative flex flex-col justify-center items-center min-h-screen p-4 sm:p-8">
        {/* Mobile backdrop: compact branded header with floating icons */}
        <div className="lg:hidden absolute inset-x-0 top-0 h-40 overflow-hidden bg-gradient-to-br from-[#0b1736] via-blue-800 to-purple-800">
        ...
        <div className="lg:hidden relative z-10 w-full flex justify-center pt-10 pb-6">
          <Logo variant="white" className="h-10" />
        </div>

        {subheading && (
          <p className="lg:hidden relative z-10 text-center text-xs text-gray-400 mb-2 max-w-xs">
            {subheading}
          </p>
        )}
